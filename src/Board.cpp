...
bool	Board::canMoveDown(const PuyoPair& pair) const {
	...
	if (y1 + 1 >= BOARD_HEIGHT || grid_[x1][y1 + 1].getColor() != PuyoColor::EMPTY) {
		return false;
	}
	if (y2 + 1 >= BOARD_HEIGHT || grid_[x2][y2 + 1].getColor() != PuyoColor::EMPTY) {
		return false;
	}
...
bool	Board::canMoveDown(const Puyo& puyo, const Puyo& otherPuyo) const {
	...
	if (x == otherPuyo.getX() && y + 1 == otherPuyo.getY()) {
		return otherPuyo.getY() + 1 < BOARD_HEIGHT && grid_[x][otherPuyo.getY() + 1].getColor() == PuyoColor::EMPTY;
	}

	if (y + 1 >= BOARD_HEIGHT || grid_[x][y + 1].getColor() != PuyoColor::EMPTY) {
		return false;
	}
	return true;
}
...
bool	Board::canMoveRight(const PuyoPair& pair) const {
	...
	int	x2 = pair.getSecondaryPuyo().getX();
	int	y2 = pair.getSecondaryPuyo().getY();
	if ((x1 + 1 < BOARD_WIDTH && grid_[x1 + 1][y1].getColor() == PuyoColor::EMPTY)
		&& (x2 + 1 < BOARD_WIDTH && grid_[x2 + 1][y2].getColor() == PuyoColor::EMPTY)) {
		return true;
	}
	return false;
}
...
bool	Board::canRotate(const PuyoPair& pair) const {
	...
	if ((rotate == 0 && x + 1 < BOARD_WIDTH && grid_[x + 1][y].getColor() == PuyoColor::EMPTY)
		|| (rotate == 1 && y + 1 < BOARD_HEIGHT && grid_[x][y + 1].getColor() == PuyoColor::EMPTY)
